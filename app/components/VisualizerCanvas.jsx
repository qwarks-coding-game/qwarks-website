...
export default function VisualizerCanvas({ match, ...props }) {
    ...
    const getFontForText = (text, overrideZoom = 0) => {
        const zoom =
            overrideZoom > 0 ? overrideZoom : canvasState.current.cameraZoom;
        if (zoom === 1) {
            if (text.length < 3) {
                return "32px Arial";
    ...
    const draw = ({ ctx, frameCount }) => {
        ...
        Object.entries(energySources).forEach(([loc, income]) => {
            var [x, y] = loc.split(",");
            var xPos =
                (x - 0.5) * DRAW_SIZE - canvasState.current.cameraOffset[0];
            var yPos =
                (y - 0.5) * DRAW_SIZE - canvasState.current.cameraOffset[1];
            ctx.lineWidth = Math.ceil(income / 5 / ZOOM_DIVISOR);
            ctx.strokeRect(xPos, yPos, DRAW_SIZE, DRAW_SIZE);
        });
        canvasState.current.rounds[canvasState.current.roundNum].qwarks.forEach(
            (qwark) => {
                qwark.actions.forEach((action) => {
                    ...
                    ctx.lineTo(
                        action[1][0] * DRAW_SIZE -
                            canvasState.current.cameraOffset[0],
                        action[1][1] * DRAW_SIZE -
                            canvasState.current.cameraOffset[1]
                    );
                    ctx.stroke();
                });
        ...
        canvasState.current.rounds[canvasState.current.roundNum].qwarks.forEach(
            (qwark) => {
                ...
                var baseX =
                    qwark.x * DRAW_SIZE - canvasState.current.cameraOffset[0];
                var baseY =
                    qwark.y * DRAW_SIZE - canvasState.current.cameraOffset[1];
                ...
                ctx.globalAlpha = 1 / 4;
                var size =
                    DRAW_SIZE + (Math.sqrt(qwark.energy) * 5) / ZOOM_DIVISOR;
                ctx.beginPath();
                ...
                ctx.fillText(
                    ...
                    baseY +
                        (renderSize.actualBoundingBoxAscent +
                            renderSize.actualBoundingBoxDescent) /
                            2
                );
        ...
        ctx.font = "16px Arial";
        ctx.fillText(
            `Round: ${roundNum}/${canvasState.current.rounds.length}`,
            10,
            20
        );
    };
    ...
    useEffect(() => {
        ...
        const handleKeyUp = (e) => {
            ...
            if (e.key.toLowerCase() === "z") {
                ...
                var ZOOM_DIVISOR = 2 ** (canvasState.current.cameraZoom - 1);
                var DRAW_SIZE = Math.floor(50 / ZOOM_DIVISOR);
                ...
                ZOOM_DIVISOR = 2 ** (canvasState.current.cameraZoom - 1);
                DRAW_SIZE = Math.floor(50 / ZOOM_DIVISOR);

                // Recalculate camera offset to maintain center point
                canvasState.current.cameraOffset[0] =
                    centerX * DRAW_SIZE - canvasSize.width / 2;
                canvasState.current.cameraOffset[1] =
                    centerY * DRAW_SIZE - canvasSize.height / 2;
            }
            ...
            if (
                e.key.toLowerCase() === "r" &&
                canvasState.current.rounds[canvasState.current.roundNum]?.qwarks
                    ?.length > 0
            ) {
                // Center camera
                const canvasSize = canvasRef.current.getBoundingClientRect();
                const ZOOM_DIVISOR = Math.pow(
                    2,
                    canvasState.current.cameraZoom - 1
                );
                const DRAW_SIZE = Math.floor(50 / ZOOM_DIVISOR);
                canvasState.current.cameraOffset = [
                    canvasState.current.rounds[canvasState.current.roundNum]
                        .qwarks[0].x *
                        DRAW_SIZE -
                        canvasSize.width / 2,
                    canvasState.current.rounds[canvasState.current.roundNum]
        ...
        const handleMouseMove = (e) => {
            ...
            canvasState.current.mousePos = [
                ((e.clientX - rect.left) * scaleX) / ratio,
                ((e.clientY - rect.top) * scaleY) / ratio,
            ];
        };
